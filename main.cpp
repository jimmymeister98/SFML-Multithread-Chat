...
void heartbeat(std::vector<sf::TcpSocket*> &clientlist) //TODO Implement with multithreading
{
    while(true){
    ...
    while (!quit)
    {
        if (selector.wait());
        {
            ...
            if (selector.isReady(listener))
            {
                ...
                std::lock_guard<sf::Mutex> lock(globalMutex);           //locks mutex, unlocks if out of scope
                //globalMutex.lock();
...
int main(int argc, char* argv[])
{
    ...
    if ((PORT == 5000) || (PORT == 5001)) {
        ...
        thread = new sf::Thread(&DoStuff, Name);
        thread->launch();
